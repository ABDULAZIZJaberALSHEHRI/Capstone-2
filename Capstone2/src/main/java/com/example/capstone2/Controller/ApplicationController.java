...
public class ApplicationController {
    ...
    public ResponseEntity AddApplication(@Valid @RequestBody Application application, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity UpdateApplication(@PathVariable int applicationid,@Valid @RequestBody Application application,Errors errors) {
        if (errors.hasErrors()) {
            String message = errors.getAllErrors().get(0).getDefaultMessage();
            return ResponseEntity.status(400).body(message);
        }
